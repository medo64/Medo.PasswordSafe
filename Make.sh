...
function debug() {
    ...
    dotnet build "$BASE_DIRECTORY/src/PasswordSafe.sln" \
                 --framework "net7.0" \
                 --configuration "Debug" \
                 --output "$BASE_DIRECTORY/build/debug/" \
                 --verbosity "minimal" \
                 || return 1
    cp "$BASE_DIRECTORY/build/debug/PasswordSafe.dll" "$BASE_DIRECTORY/bin/" || return 1
    cp "$BASE_DIRECTORY/build/debug/PasswordSafe.pdb" "$BASE_DIRECTORY/bin/" || return 1
...
function release() {
    ...
    dotnet build "$BASE_DIRECTORY/src/PasswordSafe.sln" \
                 ...
                 --verbosity "minimal" \
                 || return 1
    cp "$BASE_DIRECTORY/build/release/PasswordSafe.dll" "$BASE_DIRECTORY/bin/" || return 1
    cp "$BASE_DIRECTORY/build/release/PasswordSafe.pdb" "$BASE_DIRECTORY/bin/" || return 1
    echo "${ANSI_CYAN}Output in 'bin/'${ANSI_RESET}"
}
